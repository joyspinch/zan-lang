param([int]$X=705,[int]$Y=470,[string]$Comp="Button",[string]$State="",[string]$Out="d:\project\zan-lang\build\hover.png")
...
# Move the cursor onto the target button to trigger WM_MOUSEMOVE hover.
[WG]::SetCursorPos($X-6,$Y) | Out-Null
Start-Sleep -Milliseconds 150
[WG]::SetCursorPos($X,$Y) | Out-Null
[WG]::mouse_event(0x0001,0,0,0,[IntPtr]::Zero)
...
$bmp.Save($Out)
Write-Output ("ok size=" + $w + "x" + $ht + " cursor=" + $X + "," + $Y)
