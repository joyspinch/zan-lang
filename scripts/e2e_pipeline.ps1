...
foreach ($mf in $manifests) {
    ...
    # gui-empty / gui-components are designer projects: their code-behind is a
    # partial class whose typed control fields live in the <Name>.g.zan the
    # form designer generates from the .zform, so they only compile inside the
    # IDE and are covered there, not here.
    $srcFiles = @(Get-ChildItem $tplDir -Recurse -Filter "*.zan")
    if ($srcFiles.Count -eq 0) {
        $results += "SKIP [$tplId]: manifest-only template (generated by IDE wizard)"
        continue
    }
    if (@(Get-ChildItem $tplDir -Recurse -Filter "*.zform").Count -gt 0) {
        $results += "SKIP [$tplId]: designer template (.g.zan generated by the IDE)"
        continue
    }
