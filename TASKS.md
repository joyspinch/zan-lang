...
* **A82 wasm32 后续路线（2026-09-01 盘点，本轮已修链接全断，以下为剩余）**：当前 wasm32（WASI）可编译运行的范围：CLI/计算/字符串/集合 + 文件 IO（探针在 Node WASI 下全链路验证）。剩余缺口按优先级：
  1. **try/catch（wasm-EH 后端）**：现为编译期诊断拒绝（irgen_stmt.c AST_TRY_STMT 的 wasm 分支）。根因：wasi-libc 不提供 setjmp/longjmp（头文件显式 #error），zig 20.1.2 自带 LLVM 20.1.2 的 `-wasm-enable-sjlj` 在 catchret 上后端崩溃（O0/O2 同崩，最小 C 探针 `_scratch/sjlj_test.c`），mozbuild LLVM 19（zanc 实际链接的 LLVM-C.dll，`ZAN_HAVE_LLVM_WEBASSEMBLY=1` 已开）没有 SJLJ flag 但 **支持 WebAssembly EH intrinsics**（llvm.wasm.try.catch / get.exception / get.ehselector，LLVM 12+ 落地、经 TargetMachine feature `+exception-handling,+reference-types` 启用）。可行修法：irgen 在 wasm 三元组上把 try/catch 降级为 wasm-EH IR（try body/catch block 映射 LLVM `landingpad`-style intrinsics + `catchret`），TargetMachine feature 位开 exception-handling；jmp_buf 槽（ZAN_EH_SLOT_BYTES 1040）在 wasm 上退化为"全局异常寄存器槽"，throw 用 `llvm.wasm.throw`。工作量：中（一块独立 irgen EH 降级层 + wasi-libc 需带 EH 重建或绕开）；运行引擎需支持 EH 提案（wasmtime 14+/Node 20+ 已支持，Node 22 全量）。修掉后 stdlib 59 个含 try 文件在 wasm 上才真正可用（auto-stdlib 下含 throw 声明的模块目前靠 fail-soft 路径无 EH 引用躲过链接，但用户代码一写 try 就被诊断拒绝）。
...
* **A83 场景设计器三连批：窗口属性全链路 + on* 事件声明式动作 + IGameScene 适配器（已完成）**（2026-09-02）：用户报告「窗口初始化位置等基础属性好像没用」，排查发现是"半通"状态——每个环节只对上自己那一半，设计器改的属性到不了运行时。三批收口（每批独立提交）：
