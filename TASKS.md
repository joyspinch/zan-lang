...
* **A80 zanc 退出段错误（flaky，exit-path LLVM teardown，待查）**（2026-08-31）：大型 GUI 模块（--auto-stdlib 拉入 Gui 全量，约 330 文件）编译完成后进程退出时段错误，`Compiled 330 files` 之后；~20-40% 复现率，console 小程序不复现；fd19375a 记录的「链接完成后退出时段错误」即此。cdb 实锤栈形（zanc+0xe96b96 RVA 随重链漂移，字节特征稳定 `4d 89 01 mov [r9],r8`，r9=0x2b7de4ba840 为垃圾指针）：`common_exit → _execute_onexit_table → ~LLVMContext → LLVMContextImpl::~ → teardown walker（逐项调 0x…b60）`——main() 内 zan_irgen_destroy 已 LLVMContextDispose，退出表里仍走一遍 context 析构并踩悬垂条目。ASan 构建（clang 19.1.5 GNU 前端，16 次 GUI 模块编译）零复现零报告，疑布局/分配器差异掩盖；需 ASan 下用 --auto-stdlib gallery 级输入 + 多轮采样或 PageHeap 复抓。先于本项发现的 **确定性** c00000fd 家族（emit_stmt 单帧 ~6KB × stdlib 深 else-if 链 Theme.SetToken(100)/IconVector.Draw(256) 超 1MB 默认栈，zform 模板 100% 复现）已以 zanc 链接选项 /STACK:32MB 修复，与本 UAF 无关。
